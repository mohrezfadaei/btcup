...
@pytest.fixture
def influx_writer():
    return InfluxDBWriter(influx_url='http://localhost:8086', influx_token='test_token', influx_org='test_org')
...
async def test_write_data(mocker, influx_writer):
    ...
    mocker.patch('app.utils.influxdb_writer.InfluxDBClientAsync', return_value=mock_influx_client)
    
    formatted_data = 'currency_data,btc_rate=30000.0 1627584010000000000'
    await influx_writer.write_data(formatted_data)

    mock_write_api.write.assert_called_once_with(bucket='currency', record=formatted_data)
